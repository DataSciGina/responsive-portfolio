...
// Detectar idioma y cargarlo
function detectAndLoadLanguage() {
    // ¿El usuario ya eligió un idioma antes?
    ...
    if (savedLang) {
        ...
        const browserLang = navigator.language.slice(0, 2); // 'es', 'en'
        const supportedLangs = ['en', 'es']; // Idiomas disponibles
        const langToUse = supportedLangs.includes(browserLang) ? browserLang : 'en'; // idioma por defecto
...
languageSwitcher.addEventListener('change', (e) => {
    ...
    localStorage.setItem('preferredLanguage', selectedLang); // Guardar preferencia
    loadLanguage(selectedLang)
});
...
new Typewriter('#typewriter', {
  strings: ['Agostina Fernández', 'Data Scientist', 'Machine Learning Engineer', 'Data Analyst', 'Data Engineer', ''],
  autoStart: true,
  loop: true,
  cursor: "|"
});
